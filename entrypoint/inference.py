...
# Import XLNet Hybrid Evaluator
try:
    from hybrid_evaluator import get_hybrid_evaluator
    XLNET_AVAILABLE = True
    print("✅ XLNet hybrid evaluator loaded successfully")
except Exception as e:
    XLNET_AVAILABLE = False
    print(f"⚠️  XLNet evaluator not available: {e}")
...
@app.post("/api/evaluate/xlnet")
async def evaluate_xlnet(payload: dict):
    """
    Evaluate answer using XLNet hybrid system (fast + detailed)
    Request: {
    ...
    if not XLNET_AVAILABLE:
        return JSONResponse({"error": "XLNet evaluator not available"}, status_code=503)
    ...
    try:
        ...
        result = await evaluator.evaluate_answer(
            ...
            use_detailed_feedback=data.get("detailed", True)
        )
...
async def evaluate_answer(payload: dict = None, db=Depends(get_db)):
    ...
    # Try XLNet Hybrid Evaluator first (fastest and most comprehensive)
    evaluation = None
    evaluation_source = "unknown"
    
    if XLNET_AVAILABLE:
        try:
            xlnet_evaluator = get_hybrid_evaluator()
            xlnet_result = await xlnet_evaluator.evaluate_answer(
            ...
            # Convert XLNet result to standard format
            feedback = xlnet_result.get('detailed_feedback', {})
            evaluation = {
                "summary": feedback.get('suggestions', 'Answer evaluated'),
                "improvements": feedback.get('improvements', []),
                "strengths": feedback.get('strengths', []),
                "grammar": "",
                "fillers": "",
                "score": xlnet_result['score'] / 10,  # Convert 0-100 to 0-10
                "confidence": xlnet_result['confidence'],
                "breakdown": xlnet_result['breakdown'],
                "quick_analysis": xlnet_result['quick_analysis'],
                "example_answer": feedback.get('example_answer', '')
            }
            evaluation_source = xlnet_result.get('evaluation_method', 'xlnet_hybrid')
            logger.info(f"Answer evaluated using XLNet hybrid: score={xlnet_result['score']}")
        except Exception as e:
            logger.error(f"XLNet hybrid evaluation failed: {e}")
            evaluation = None
    
    # Fallback to Hugging Face if XLNet not available
    if evaluation is None and HF_AVAILABLE:
        try:
